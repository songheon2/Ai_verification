...
def inspect_custom(path: str) -> ModelInfo:
    ...
    return ModelInfo(
        ...
        warnings=[
            "custom format files do not retain ONNX activation or preprocessing metadata"
        ],
    )
...
def inspect_model(path: str) -> ModelInfo:
    ...
    if suffix in {".bin", ".txt", ".custom"}:
        return inspect_custom(path)
    raise ValueError("model must be an .onnx, .bin, .txt, or .custom file")
