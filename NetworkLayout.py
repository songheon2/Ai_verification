...
단독 실행 (데모/스모크 테스트용)
--------------------------------
    python NetworkLayout.py [--model model.bin] [--output output.png]
    ...
    python NetworkLayout.py --select-mode threshold --threshold 5 --model "Custom/model.bin"
...
def main() -> None:
    ...
    parser.add_argument("--output", "-o", default=None, help="출력 이미지 경로 (생략 시 <model파일명>_layout_demo.png)")
    ...
    output_path = args.output or (os.path.splitext(model_path)[0] + f"_layout_demo_{args.select_mode}.png")
