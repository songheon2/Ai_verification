...
사용법:
  python -m visualization.VisualizeSparsity <custom.bin> [output] [--tol <threshold>] \
...
def visualize_node(filepath: str, output_path: str | None = None, title: str | None = None,
    ...
    if output_path is None:
        ...
        output_path = os.path.join(OUTPUT_DIR, base + "_sparsity.png")
...
def visualize_elementwise(filepath: str, output_dir: str | None = None,
    ...
    output_dir을 지정하지 않으면 visualization/outputs 아래에 저장한다.
    ...
    if output_dir is None:
        base = os.path.splitext(os.path.basename(filepath))[0]
        output_dir = os.path.join(OUTPUT_DIR, base + "_sparsity_elements")
...
if __name__ == "__main__":
    ...
    if len(argv) < 1:
        print("사용법: python -m visualization.VisualizeSparsity <custom.bin> [output] [--tol <t>] "
              "[--mode node|element] [--px-scale N]")
        sys.exit(1)
